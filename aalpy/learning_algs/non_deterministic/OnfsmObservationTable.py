        ...
        if self.trace_tree:
            for prefix in self.S.copy():
                ...
                for a in self.A:
                    trace = self.sul.pta.get_single_trace(curr_node, a)
                    for t in trace:
                        entry = (prefix[0] + a, prefix[1] + tuple(t))
                        if t and not (self.row_to_hashable(entry) in state_distinguish.keys()):
                        ...
                        if t and self.row_to_hashable(entry) in state_distinguish.keys():
                            state_in_S = state_distinguish[self.row_to_hashable(entry)]
            ...
            for prefix in self.S:
                for a in self.A:
                    for t in self.T[prefix][a]:
                        entry = (prefix[0] + a, prefix[1] + tuple([t]))
                        if self.row_to_hashable(entry) in state_distinguish.keys():
                            state_in_S = state_distinguish[self.row_to_hashable(entry)]
                            assert state_in_S
        ...
        for s in tmp_S:
            ...
            if hashed_s_row in hashed_rows_from_s:
                ...
                for key in tmp_both_S:
                    s_dot_a_row = (key[0][:size], key[1][:size])
                    if s != key and s == s_dot_a_row:
