...
class NonDetObservationTable:
    ...
    def update_obs_table(self, s_set=None, e_set: list = None):
        ...
        for s in update_S:
            ...
            for e in update_E:
                if not self.trace_tree_flag and e not in self.T[s].keys():
                    num_s_e_sampled = 0
                ...
                if self.trace_tree_flag:
                    tmp = self.sul.pta.get_single_trace(curr_node, e)
                if self.trace_tree_flag and (len(tmp) == 1) and (not tmp[0]):
                    #print(self.sul.pta.get_single_trace(curr_node, e), "qwertzuiop")
                    num_s_e_sampled = 0
                    while num_s_e_sampled < self.n_samples:
                        output = tuple(self.sul.query(s[0] + e))

                        #print(output[:len(s[1])], s[1], "234567890")

                        num_s_e_sampled += 1
                        #if output[:len(s[1])] == s[1]:
                        #    num_s_e_sampled += 1
                    #print(self.sul.pta.get_single_trace(curr_node, e), "poiuztrewq")
    ...
    def clean_obs_table(self):
        ...
        s = self.S
        e = self.E
        s_dot_a = self.S_dot_A

        for key in s:
            nodes[self.sul.pta.get_to_node(key[0], key[1])] = (key, [])

        # search for rows that aren't unique anymore
        for inp in e:
            known_outputs = []
            for node in nodes.keys():
                out = set(self.sul.pta.get_single_trace(node, inp))
                if out in known_outputs:
                    nodes[node][1].append(True)
                else:
                    known_outputs.append(out)
                    nodes[node][1].append(False)

        # delete rows which are not unique anymore
        node_to_move = []
        for node in nodes.keys():
            if all(nodes[node][1]):
                node_to_move.append(nodes[node][0])
                self.S.remove(nodes[node][0])

        # add rows which were removed from S to S_dot_A and delete rows which were originally derived from newly added cell from S_dot_A
        for node in node_to_move:
            for key in s_dot_a:
                size = len(node[0])
                s_dot_a_inp = key[0][:size]
                s_dot_a_out = key[0][:size]

                if node[0] == s_dot_a_inp and node[1] == s_dot_a_out:
                    self.S_dot_A.remove(key)

            self.S_dot_A.append(node)
    ...
    def row_to_hashable(self, row_prefix):
        ...
        for e in self.E:
            row_repr += (frozenset(self.sul.pta.get_single_trace(curr_node, e)),)
